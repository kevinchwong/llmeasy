...
from llmeasy.providers import OpenAIProvider, ClaudeProvider

@pytest.mark.asyncio
async def test_query_with_template(mock_openai_client):
    """Test basic template query"""
    with patch('llmeasy.providers.openai.AsyncOpenAI') as mock_openai:
        mock_openai.return_value = mock_openai_client
        llm = LLMEasy(provider='openai', api_key='test-key')
...
async def test_provider_selection():
    ...
    with patch('llmeasy.utils.config.settings', create=True) as mock_settings:
        mock_settings.anthropic_api_key = 'test-key'
        mock_settings.openai_api_key = 'test-key'
        
        claude = LLMEasy(provider='claude', api_key='test-key')
        assert isinstance(claude.provider, ClaudeProvider)
        
        openai = LLMEasy(provider='openai', api_key='test-key')
...
async def test_invalid_provider():
    ...
    with pytest.raises(ValueError) as exc_info:
        LLMEasy(provider='invalid', api_key='test-key')
